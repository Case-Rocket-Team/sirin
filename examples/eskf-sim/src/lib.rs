...
pub fn run_deterministic_simulation() -> SimulationResult {
...
mod tests {
    ...
    #[test]
    fn delayed_gps_simulation_stays_close_to_truth() {
        let result = run_deterministic_simulation();
        assert!(result.accepted_gps_updates > 50);
        assert!(
            (result.final_position_ned_m - result.true_position_ned_m).norm() < 1.0,
            "position estimate {:?}, truth {:?}",
            result.final_position_ned_m,
            result.true_position_ned_m
        );
        assert!(
            (result.final_velocity_ned_mps - result.true_velocity_ned_mps).norm() < 0.5,
            "velocity estimate {:?}, truth {:?}",
            result.final_velocity_ned_mps,
            result.true_velocity_ned_mps
        );
    }
}
