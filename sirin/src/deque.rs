//! This is a copy/paste of a heapless::deque with some added methods. I need to refer
//! to each header by its index and I don't want to rewrite a ring buffer when I don't have
//! to.
...
impl<T, const N: usize> Deque<T, N> {
    ...
    pub fn get(&self, i: usize) -> Option<&T> {
        if self.has(i) {
            unsafe {
                Some(self.buffer.get(i).unwrap().assume_init_ref())
            }
        } else {
            None
        }
    ...
    pub fn get_mut(&mut self, i: usize) -> Option<&mut T> {
        if self.has(i) {
            unsafe {
                Some(self.buffer.get_mut(i).unwrap().assume_init_mut())
            }
        } else {
            None
        }
    ...
    /// Returns back the item if `i` is not at the start or the end (or cannot
    /// be made to be at the start or end)
